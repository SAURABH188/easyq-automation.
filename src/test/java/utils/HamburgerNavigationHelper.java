        ...
        try {
            ...
            Object clicked = ((JavascriptExecutor) driver).executeScript(
                    "const pattern = new RegExp(arguments[0], 'i');"
                            ...
                            + "let roots = Array.from(document.querySelectorAll('aside,nav,[role=\"navigation\"],.sidebar,.side-bar,.sidenav,.drawer,.navigation,.navbar,[role=\"menu\"],.dropdown-menu,.mat-menu-panel,.cdk-overlay-pane,.popover'))"
                            + "  .filter(visible);"
                            + "const sidebarRoots = Array.from(document.querySelectorAll('aside,nav,[role=\"navigation\"],div')).filter(el => {"
                            + "  if (!visible(el)) return false;"
                            + "  const rect = el.getBoundingClientRect();"
                            + "  const text = textOf(el);"
                            + "  return rect.left <= 430 && rect.width <= 470 && rect.height >= 160"
                            + "    && /dashboard/i.test(text)"
                            + "    && /quality\\s*policy/i.test(text)"
                            + "    && /quality\\s*objective/i.test(text)"
                            + "    && /responsibility\\s*(?:&|and)?\\s*authority/i.test(text);"
                            + "});"
                            + "roots = Array.from(new Set(roots.concat(sidebarRoots)));"
                            + "const scoreOf = el => {"
                            + "  const text = textOf(el);"
                            + "  const rect = el.getBoundingClientRect();"
                            + "  let score = 0;"
                            ...
                            + "const targetFor = match => {"
                            + "  let node = match;"
                            + "  let best = null;"
                            + "  while (node && node !== document.body) {"
                            + "    const rect = node.getBoundingClientRect();"
                            + "    const text = textOf(node);"
                            + "    if (visible(node) && rect.left <= 430 && rect.width >= 120 && rect.width <= 470"
                            + "        && rect.height >= 24 && rect.height <= 100 && pattern.test(text) && !blocked.test(text)) {"
                            + "      best = node;"
                            + "    }"
                            + "    node = node.parentElement;"
                            + "  }"
                            + "  return best || match.closest('a,button,[role=\"button\"],[role=\"link\"],[role=\"menuitem\"],li') || match;"
                            + "};"
                            ...
                            + "  if (match) {"
                            + "    return clickTarget(targetFor(match));"
                            + "  }"
                            + "}"
                            + "return false;",
                    menuRegex);
